...
parser.add_argument('action', help='Action from cromdriver : `get`, `del`, `list`, `test`')
...
parser.add_argument('-p', '--platform', help='Platform of chromedriver (linux, darwin or win)')
...
if args.action == 'get':
    ...
    print('Last release on your machine : {}'.format(cromdriver.get_latest_release_file()))
    print('Last release on http://chromedriver.storage.googleapis.com/index.html : {}'.format(cromdriver.get_latest_release_web()))

elif args.action == 'del':
    if args.version is not None:
        directory_to_delete = os.path.join(cromdriver.APP_DATA, 'RELEASE', args.version)
